## 状態決め
 ...
 # エンティティデータにattribute的なのを代入
  $data merge entity @s {item:{id:acacia_button,components:{item_model:"$(model)"}},teleport_duration:1,shadow_radius:0.1,shadow_strength:1,data:{"speed":"$(speed)","damage":"$(damage)","head_damage":"$(head_damage)","gravity":"$(gravity)","tick":"$(tick)","hit":"$(hit)","delete_time":$(delete_time)}}
 # 弾丸の向きをオーナーと一致させる
  rotate @s ~ ~